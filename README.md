...
`assets/img/shots/` 안의 파일은 실제 스크린샷으로 교체할 자리입니다.
파일을 넣고 `assets/js/config.js` 의 `images` 경로만 고치면 됩니다.
...
| `jobs.*` | 홈 · 직업 카드 4종 | 800×600 |

공지와 경기는 관리자 페이지에서 이미지 주소를 개별 지정할 수도 있습니다.
