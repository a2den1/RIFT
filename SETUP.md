...
`ALL.sql` 이후에 만든 것이라 **따로 한 번 실행해야 합니다.**

- `add-mc-verify.sql` — `/웹인증` 코드
- `add-auto-link.sql` — 게임에서 이미 연동한 사람 자동 인증
- `add-modal-and-job.sql` — 진입 안내 모달, 직업 쓰기 권한 회수

### 프로필과 구단에 적용되는 규칙

`add-profiles.sql` 은 화면이 아니라 DB 에서 규칙을 강제합니다.

- 경험치(`xp`)와 소속(`club`) 은 **브라우저에서 수정할 수 없습니다.** 열 단위 GRANT 로 막혀 있고,
  경험치는 마인크래프트 서버가 `service_role` 키로, 소속은 이적 함수만 바꿉니다.
- 직업(`job`) 은 게임 안에서 한 번 고르면 바꿀 수 없습니다. `add-modal-and-job.sql` 이 브라우저의
  쓰기 권한을 회수하고, 표시값은 마인크래프트 서버가 올린 `players.job` 을 씁니다.
- 구단 생성은 닉네임 · 직업 · 로고가 모두 있어야 하고, 한 사람이 두 구단을 가질 수 없습니다.
- 이적 승인은 **해당 구단의 구단주 또는 관리자만** 가능합니다.
- 구단주는 탈퇴할 수 없습니다.
